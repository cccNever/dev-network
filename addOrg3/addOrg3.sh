...
function Org3Up () {
  ...
  if [ "${DATABASE}" == "couchdb" ]; then
    ...
    docker volume create peer1.org3.example.com
...
function addOrg3 () {
  ...
  docker exec cli-node1 ./scripts/org3-scripts/updateChannelConfig.sh $CHANNEL_NAME $CLI_DELAY $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
    fatalln "ERROR !!!! Unable to create config tx"
  fi

  infoln "Joining Org3 peers to network"
  docker exec cli-node1 ./scripts/org3-scripts/joinChannel.sh $CHANNEL_NAME $CLI_DELAY $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
