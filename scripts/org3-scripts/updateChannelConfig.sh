...
infoln "Submitting transaction from a different peer (peer1.org2) which also signs it"
setGlobals 2
set -x
peer channel update -f org3_update_in_envelope.pb -c ${CHANNEL_NAME} -o orderer1.ordererOrg1.example.com:7155 --ordererTLSHostnameOverride orderer1.ordererOrg1.example.com --tls --cafile "$ORDERER_CA"
{ set +x; } 2>/dev/null
